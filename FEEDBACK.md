...
- [x] Почему бы не сделать не на GH actions, а на Space automation? - мы могли бы сделать там ревью.
    ...
    - В процессе выполнения задачи в `Space automation` запускается докер с нодами хрома, выполняются тесты и формируется отчет, который загружается как артефакт в файловый репозиторий Space.
